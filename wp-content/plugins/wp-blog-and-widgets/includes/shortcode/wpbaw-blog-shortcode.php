...
function wpbaw_get_blog_post( $atts, $content = null ){
	...
	<?php  
		...
		<div id="post-<?php the_ID(); ?>" class="blog type-blog blog-clearfix <?php echo ($feat_image) ? "has-thumb" : "no-thumb";?> blog-col-<?php echo $grid.' '.$css_class.' '.$date_class; ?>">
			<div class="blog-inner-wrap-view blog-clearfix card">
				<?php if ( $feat_image ) { ?>
					<div class="blog-thumb">
						<div class="grid-blog-thumb">
							<a href="<?php the_permalink(); ?>"><img class="wpbaw-blog-img" src="<?php echo esc_url($feat_image); ?>" alt="<?php echo $post_title; ?>"/></a>
						</div>
                        <div class="blog-date"><?php echo ($show_date)? date('M j Y', strtotime(get_the_date())) : "" ;?></div>
					</div>
				<?php } ?>
				<div class="blog-content card-body pt-0">
					<?php if($grid == '1') {
						if($show_date){?>
							<div class="date-post">
								<h4><span><?php echo get_the_date('j'); ?></span></h4>
					...
					<?php } else { ?>
						...
						<?php if($show_author || $show_date){ ?>
							<div class="blog-author">
								<?php if($show_author) {?>
									<span>
										<i class="las la-pen mr-2"></i><?php/* esc_html_e( 'By', 'wp-blog-and-widgets' ); */?><a class="text-body" href="<?php echo esc_url( get_author_posts_url( $post->post_author ) ); ?> " target="_self"><?php echo get_the_author(); ?></a>
									</span>
								<?php }?>
					...
					<div class="post-content-text">
						...
						<?php } ?>
						<?php if($post_title) { ?>
							<h5 class="blog-title"><a class="text-primary" href="<?php the_permalink(); ?>" rel="bookmark"><?php echo $post_title; ?></a></h5> 
						<?php } ?> 
						<?php if($grid == '1'){ ?>
							<div class="blog-cat">
						...
						<?php if($show_content){ ?>
							<div class="blog-content-excerpt">
								<?php  if( !$show_full_content ) { ?>
									...
									<a href="<?php the_permalink(); ?>" class="blog-more-link1 text-dark text-sm"> <?php _e( 'Read More', 'wp-blog-and-widgets' ); ?> <i class="las la-long-arrow-alt-right mr-2"></i> </a>
								<?php } else {
